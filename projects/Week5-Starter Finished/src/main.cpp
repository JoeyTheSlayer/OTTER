...
#include "OBJLoad.h"
// #include "OBJLoader.h" <-added by Jose
...
int main() {
	...
	VertexArrayObject::sptr vao5 = OBJLoad::LoadFile("monkey.obj");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//tKeyWatcher.Poll(window); <-commented out Jose
		...
		/* Jose - Check this start
		if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS && cameraa == false)
		{
			cameraa = true;
			camera->SetOtho(true);
		}
		else if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_RELEASE && cameraa == true)
		{
			cameraa = false;
			camera->SetOtho(false);
		}
		// Jose - check this end */
		...
		*/
		...
		transform2 = glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f)) * transform;

		/* Commented by Jose */
		transform4 = transform * glm::translate(glm::mat4(1.0f), glm::vec3(0, 0.0f, glm::sin(static_cast<float>(thisFrame)))); 
