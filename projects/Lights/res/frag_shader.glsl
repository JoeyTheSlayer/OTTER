...
void main() {
    ...
    vec3 halfDir = vec3(normalize(lightDir + camDir));
	vec3 reflectDir = reflect(-lightDir, N);
    float spec = pow(max(dot(N, halfDir), 0.0), 128); // Shininess coefficient (can be a uniform)    vec3 specular = specularStrength * spec * lightColor; // Can also use a specular color
